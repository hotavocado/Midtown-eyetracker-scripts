...
isempty(strmatch('y', subj_ID(6,1),'exact')); 
            ...
            if m==1 & s==1
                ...
                % if par.useEL, calllib('iViewXAPI', 'iV_SendImageMessage', formatString(256, int8([ num2str(num2str(par.CD_TGOFF))])));end;
                 %if par.useEL, [success, ivx]=iViewX('message', ivx, [ num2str(num2str(par.CD_TGOFF))]);end;
                   if par.useEL, eyetr_sendtrigger([ num2str(num2str(par.CD_TGOFF))],sock); end;
        ...
        if s==1
            ...
            %if par.useEL, calllib('iViewXAPI', 'iV_SendImageMessage', formatString(256, int8([ num2str(num2str(par.CD_TG(trialLR(n))))])));end;
             %if par.useEL, [success, ivx]=iViewX('message', ivx, [ num2str(num2str(par.CD_TG(trialLR(n))))]);end;
              if par.useEL, eyetr_sendtrigger([ num2str(num2str(par.CD_TG(trialLR(n))))],sock); end;
...
Screen('DrawText', window, 'Nice Job!', 0.35*scresw, 0.55*scresh, 255);
Screen('Flip', window);  
WaitSecs(3)
...
% [success, ivx]=iViewX('datafile', ivx, ['C:\PsychToolbox_Experiments\Simon\HBN\AA_eyetracker_data\' subj_ID{1,1} '_SAIIT_2AFC_Block3'  '.idf']);
% 
...
save([par.runID , '_SAIIT_2AFC_Block3'],'ITIstartT','TargOnT','RespT','RespLR','trialITI','trialLR','par') 
close all
