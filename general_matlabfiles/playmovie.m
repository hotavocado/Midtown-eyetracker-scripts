function [dur, delay] = playmovie(movie, trg1, trg2, ivx, n)
...
% send fake trigger
NetStation('Event', num2str(9999));
%[success, ivx]=iViewX('message', ivx, num2str(9999));
if par.useEL, eyetr_sendtrigger(num2str(9999),sock); end;
...
Screen('PlayMovie',moviePtr,1,0,1.0);
tstart = GetSecs;
NetStation('Event', trg1);
%[success, ivx]=iViewX('message', ivx, trg1);
if par.useEL, eyetr_sendtrigger(trg1,sock); end;
...
while 1
    ...
    elseif tex<0
        ...
        t0 = GetSecs;
        NetStation('Event', trg2);
        %[success, ivx]=iViewX('message', ivx, trg2);
        if par.useEL, eyetr_sendtrigger(trg2,sock); end;
        fprintf(1, 'trigger delay %2.4f\n', GetSecs-t0);
        tend = GetSecs;
        break;
    end
